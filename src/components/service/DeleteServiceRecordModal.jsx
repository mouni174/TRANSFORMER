...
export const DeleteServiceRecordModal = ({ isOpen, record, onClose, onConfirm, isDeleting }) => {
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-5 border-b border-slate-100 flex items-start space-x-3 bg-red-50/50">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-sm font-bold text-slate-900">Confirm Service Record Deletion</h3>
        ...
        <div className="p-5 space-y-3 text-xs text-slate-700">
          <p>
            Are you sure you want to delete the service record for transformer{' '}
            <span className="font-bold text-slate-900 font-mono">{record.serialNumber}</span> performed on{' '}
            <span className="font-bold text-slate-900">{formatDisplayDate(record.service_date)}</span>?
          </p>
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-md text-amber-800 text-[11px] leading-relaxed">
            <span className="font-semibold block mb-0.5">Historical Record Deletion:</span>
            Deleting this record will permanently remove the historical service entry and its associated 1-year warranty record from the database.
          </div>
        </div>
